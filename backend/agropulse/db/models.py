...
class Field(Base):
    ...
    # Культура текущего сезона. Обязательна при создании поля через API,
    # но колонка остаётся nullable: поля, заведённые до введения этого правила,
    # существуют, и подставлять им выдуманное значение ради ограничения на
    # уровне хранения означало бы записать в данные то, чего никто не сообщал.
...
class Anomaly(Base):
    ...
    #
    # Одно поле вместо трёх прежних. `confidence` был длительностью, переодетой
    # в уверенность: доля восстановленных значений равна примерно 0,8 у любого
    # события по построению, а «число точек» считало дни суточной сетки.
    # `corroboration` списывал облачное окно трижды и выносил «слабо» событию,
    # на котором сошлись радар и погода. Понижение уверенности при несовпадении
    # фазы сезона было третьим показателем и пользователю не показывалось вовсе.
    #
    # Числа нет намеренно: веса прежней суммы были назначены, а не откалиброваны,
    # и «35 из 100» выглядело измерением, будучи мнением. Основания вердикта
    # лежат в `factors` под ключом `trust`.
    trust: Mapped[str | None] = mapped_column(String(20))
