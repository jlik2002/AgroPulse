...
class Settings(BaseSettings):
    ...
    # --- модель V6 (восстановление пропусков) ---
    ml_v6_enabled: bool = True
    """Использовать локальную модель V6 вместо обращения к сервису моделей.

    Модель обучена заранее, её артефакты лежат рядом с кодом; в сервисе
    выполняется только предсказание. Обучение здесь не запускается никогда.
    """

    ml_v6_artifacts_dir: Path = Path(__file__).resolve().parent / "ml" / "v6" / "artifacts"
    """Каталог с обученной моделью. По умолчанию — тот, что в образе."""

    ml_v6_reference_dataset: Path = Path("/app/data/train_dataset.csv")
    """Опорный набор задачи 1 (`train_dataset.csv`), контекст для признаков.

    Обязателен. Признаки V6 межполевые: доноры, эффект дня, преобразование
    съёмки — поле сравнивается с другими полями той же даты. Ряд, поданный
    в одиночку, сравнивать не с чем, поэтому без набора модель не считает
    вовсе, а не считает хуже. Если набора нет, `ml/resolve.py` сообщает об
    этом в журнал и модель не используется.

    В репозиторий набор не входит (см. `data/README.md`): он кладётся
    в смонтированный каталог обмена, отсюда и путь `/app/data/...`.
    """
    ...
    openrouter_max_tokens: PositiveInt = 6000
    """Бюджет ответа модели в токенах.

    Это предохранитель от бесконечной генерации, а не способ задать длину
    текста. Рассуждающие модели тратят на внутренние рассуждения кратно
    больше, чем на сам ответ, и расходуют они тот же бюджет: замер на
    `qwen3.8-27b` дал 2000 токенов рассуждения при нулевом ответе. Длина
    текста задаётся отдельно — `llm_answer_max_words`.
    """

    llm_answer_max_words: int = Field(default=90, ge=20, le=400)
    """Ограничение длины пояснения, слов.

    Уходит в промпт, а не в `max_tokens`. Разница принципиальная: обрезание
    по токенам даёт оборванный на полуслове текст, который сверка чисел
    законно бракует целиком, а указание в промпте заставляет модель сразу
    писать короче.
    """
