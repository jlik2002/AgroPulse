# ВНИМАНИЕ КОНЦЕНТРАЦИЯ AI SLOP ПРЕВЫШАЕТ ВСЕ ДОПУСТИМЫЕ НОРМЫ
...
Задача Космохакатона — мониторинг вегетационной динамики сельхозполей:
веб-сервис, который по выбранному полигону сам собирает спутниковые
и метеоданные, восстанавливает пропуски в ряде NDVI и показывает аномальные
периоды, плюс batch-инференс `private_features.csv → submission.csv`.

Это хакатонный код: писался на скорость и под критерии оценки, а не под
эксплуатацию. Что работает плохо:

- **CatBoost на восстановлении пропусков.** Предсказывает не NDVI, а поправку
  к линейной интерполяции — основную работу делает опора, не модель. Главный
  выигрыш даёт приведение трёх приборов к общей шкале, остальное добавляет
  мало. На поле, которого не было в обучении, межполевые признаки вырождаются
  и качество падает молча.
- **Детекция аномалий — на тоненького.** Пороги подобраны на четырёх полях,
  одно вплотную к границе. Севооборот путается с угнетением, уборка урожая
  от него не отделена, precision и recall не измерены.
- **Попиксельного анализа нет.** Поле сводится к одному числу на дату, поэтому
  локализации аномалии внутри поля нет, а `anomalous_area_fraction` подменена
  более слабой величиной.
- **Радар не закрывает облачность.** Ряд показывается, но восстановление NDVI
  по радару не реализовано.

Развёрнутого стенда больше нет — сервер удалён. Разворачивается локально
по инструкции ниже.
