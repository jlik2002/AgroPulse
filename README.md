...
2. **Проверка параметров** — выбор полей для расчёта, площадь, источники
   данных и предупреждения. По умолчанию отмечены ещё не посчитанные поля:
   сбор по каждому занимает минуты, и добавив одно поле к десяти готовым,
   пересчитывать все незачем. Поле без указанной культуры запуск блокирует.
...
7. **Отчёты** — выбор разделов, сборка PDF и предпросмотр готового документа.
   До сборки предпросмотр пуст: показывать вместо документа его подобие
   значило бы показывать не то, что скачает пользователь.
...
идентификатором и приписывает к нему создаваемые проекты, поэтому после
закрытия браузера пользователь возвращается к своей сводке и отчётам.
Проверкой прав это не является: идентификатор проекта остаётся ключом
доступа к нему, и ссылка на результаты открывается у любого, кому её
отправили, — так и задумано.
...
# 2. Поле: полигон GeoJSON и культура текущего сезона обязательны,
#    дата посева — нет
...
Модели нужен опорный набор `train_dataset.csv` в том же каталоге — без него
она объявляет себя недоступной (см. [`data/README.md`](data/README.md)).
...
ml-service       внешний сервис моделей, запасной путь при ML_V6_ENABLED=false
```
...
Пропуски NDVI восстанавливает модель **V6**: CatBoost предсказывает поправку
к линейной опоре, приведённой к шкале конкретного прибора, а предсказания трёх
...
Модель обучена заранее, её артефакты лежат в образе рядом с кодом. **Обучение
в сервисе не запускается никогда**: оно занимает около пяти минут, и здесь
выполняется только предсказание. Один и тот же клиент обслуживает и веб-сценарий,
и batch-инференс — расхождение между ними означало бы, что проверяется одно,
а работает другое.

V6 нужен опорный набор `train_dataset.csv` в каталоге обмена (см.
[`data/README.md`](data/README.md)): её признаки межполевые, и ряд, поданный
в одиночку, сравнивать не с чем.
...
[`docs/ML_API.md`](docs/ML_API.md), он включается при `ML_V6_ENABLED=false`.
